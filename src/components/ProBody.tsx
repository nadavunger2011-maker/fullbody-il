        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-8">
            {herbalifeProducts
              ...
              <Link
                key={product.handle}
                href="https://shop.fullbody.co.il/products/${product.handle}"
            ...
            {searchQuery && (
              <div className="mt-4 max-h-60 overflow-y-auto">
                {filteredProducts.length > 0 ? filteredProducts.slice(0, 5).map(p => (
                  <Link key={p?.node?.id} href="https://shop.fullbody.co.il/products/${p?.node?.handle}" onClick={() => setIsSearchOpen(false)} className="flex items-center gap-3 py-3 hover:bg-secondary/50 rounded-lg px-2 transition-colors cursor-pointer">
