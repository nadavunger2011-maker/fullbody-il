    ...
    return (
      <aside className="my-8 not-prose bg-gradient-to-br from-[hsl(142,70%,97%)] to-card border-2 border-[hsl(142,70%,35%)]/20 rounded-2xl p-5 shadow-card animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center">
          <Link href="https://shop.fullbody.co.il/products/${product.handle}" className="shrink-0 bg-secondary/30 rounded-xl p-3 w-32 h-32 flex items-center justify-center">
            <img src={product.image} alt={product.title} className="max-w-full max-h-full object-contain" loading="lazy" />
          </Link>
          <div className="flex-1 text-right">
            <Link href="https://shop.fullbody.co.il/products/${product.handle}">
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              ...
              {onAddToCart ? (
                ...
                <Link href="https://shop.fullbody.co.il/products/${product.handle}" className="bg-[hsl(142,70%,35%)] text-white font-bold px-4 py-2 rounded-lg text-sm hover:opacity-90 transition-all">
                  לפרטים
                </Link>
              )}
            </div>
