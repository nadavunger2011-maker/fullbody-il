...
- ביטויים אסורים: לעולם אל תשתמש ב"בעולם המודרני של היום", "חשוב לציין", "לסיכום".
- אל תייצר כותרות בנוסח רובוטי או שחוק כמו: "מעבר ל...", "הדרך ל...", "המדריך המלא ל...", "מעבר ל...".
- העדף כותרות קונקרטיות, חיפושיות וטבעיות בעברית – שאלה אמיתית, הבטחת ערך ברורה או פתרון לבעיה אמיתית.
- השתמש בעברית טבעית וזורמת, בלי חזרות על אותה תבנית בין מאמרים.
...
- השתמש רק ב-HTML טקסטואלי עם <figure class="infographic" role="img" aria-label="תיאור"> ו-<figcaption>.
- מותר להשתמש רק ברכיבים כמו <table>, <div>, <ul>, <ol>, <strong>, <span>.
- אסור להחזיר תגיות <img> בתוך התוכן.
- אסור להשתמש בכתובות תמונה, assets מקומיים, /images/, CDN, או כל URL חיצוני בתוך content.
...
- אל תייצר כותרות בנוסח גנרי כמו "מעבר ל..." או "המדריך המלא...".
...
- השתמש רק ב-HTML טקסטואלי; בלי <img>, בלי URLs ובלי /images/ בתוך התוכן.
